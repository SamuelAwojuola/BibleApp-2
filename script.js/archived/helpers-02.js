...
function toggleClassAndActiveButton(elm, cls, originElm) {
    if (Array.isArray(elm)) {
        elm.forEach(x => {
            ...
            if (cls == 'slideup') {
                tabIndexChangeHideORshow(x, addORremove < 1)
            }
        })
    } else {
        if (cls != 'displaynone' && cls == 'slideup' && elm.matches('.displaynone') && elm.matches('.slideup')) {
            elm.classList.remove('displaynone')
        } // Add this particularly because of titlebarsearchparameters because of tab selection
        const addORremove = elm.classList.toggle(cls)
        if (cls == 'slideup') {
            tabIndexChangeHideORshow(elm, addORremove < 1)
        }
    }
    if (originElm) {
...
function checkUncheck(x) {
    ...
    if (arrOfCheckBoxes.every(x => x.getAttribute('type') == "radio" && x.checked == true)) {
        return
    } //ensure clicking on a checked radio input does not uncheck it
...
function slideUpDown(elm, upOrDown, t) {
    ...
    if (slideUpDownTimer) {
        clearTimeout(slideUpDownTimer)
    }

    const tMargin = elm.offsetHeight;
    let animDuration = t ? (t > 300 ? 300 : t) : (tMargin * 0.8);

    if (animDuration <= 0 && (anim_dur = elm.getAttribute('anim_dur'))) {
        animDuration = anim_dur;
    } else if (animDuration < 300) {
        animDuration = 300;
    }
...
function autocomplete(e) {
    ...
    if (!e.target.matches('.v_markerinputnbtn_holder input')) {
        return
    }
    ...
    if (autocomplete_1check.checked) {
        arr = arrOfAllVerseMarkersInBook;
    } else if (autocomplete_2check.checked) {
        arr = allVMarkersInAllBooks;
    }
    let currentFocus;
    ...
    inputElm.addEventListener("keydown", function (e) {
        ...
        if (autocomplete_items) {
            x = autocomplete_items.getElementsByTagName("div");
        }
        // arrow DOWN key
        if (e.keyCode == 40) {
            currentFocus++;
...
async function codeElmRefClick(e) {
    ...
    if (![0, 2].includes(e.button) || e.target.tagName != "CODE" || e.target.matches('.cmtitlebar>code')) {
        return
    }
    e.preventDefault();
    ...
    if (document.querySelector('body').matches('#versenotepage')) {
        ...
        if (e.ctrlKey || codeElm.matches('#biblenotes_nav code[ref]') || e.button == 2) {
            ...
            if (e.button == 2) {
                ...
                // toggleNav({event:'click'},'hide');
                !e.target.closest('#context_menu') && !e.ctrlKey ? hideTime = await hideRefNav('hide', e.target.closest('#refnav_col2 > div')) : null;
            }
...
function fullBookName(bkn) {
    ...
    bible.Data.books.forEach((bkAbrv, ref_indx) => {
        if (bkAbrv.includes(bkn.toUpperCase())) {
            ...
            shortBkn = bkAbrv[0] == 'JOB' ? 'Job' : bkAbrv[1].toLowerCase().replace(/[0-9]*\s*([a-z])/, (v) => {
                return v.toUpperCase()
            }); //Make lowerCase then make first alphabet upperCase
            bkIndex = ref_indx;
        }
    });
    return {
...
function modifyBkNmeOfRef(ref) {
    let full = ref.replace(/([0-9]*\s*[a-zA-Z]*\s*[a-zA-Z])/, (nm) => {
        return fullBookName(nm).fullBkn
    });
    let short = ref.replace(/([0-9]*\s*[a-zA-Z]*\s*[a-zA-Z])/, (nm) => {
        return fullBookName(nm).shortBkn
    })
    return {
        full,
        short
...
function modifyQuotationMarks(txt) {
    ...
    // txt = txt.replace(/[\u200B\u200C\u200D\uFEFF]/g, '');//remove all nonbreaking spaces
...
function isFullyScrolledIntoView(el, parentElement, partial = false, tolerance = 10) {
    ...
    const parentRect = pE ? pE.getBoundingClientRect() : {
        top: 0,
...
async function saveToLocalDrive(file_text_content, fileName, format = 'json', userFolder) {
    ...
    if (!userFolder) {
        userFolder = currentDefaultFolder
    }
    if (!fileName) {
...
async function setDefVNFolder(x, dis) {
    if (x == 'psam') { //Pastor Sam -- Main Note
        ...
        document.querySelectorAll('.verse_note .t2.text_content').forEach(t => {
            t.classList.remove('text_content') /* ; t.id=null */
        });
        document.querySelectorAll('.verse_note .t1:not(.text_content)').forEach(t => {
            t.classList.add('text_content')
        })

        getNoteForSelectedUser();

        document.querySelectorAll('.verse_note .user1_vn_check').forEach(cb => {
            cb.checked = false
        });
        document.querySelectorAll('.verse_note .notes_ref_head').forEach(t => {
            t.classList.remove('user1')
        });
        document.querySelectorAll('.verse_note .defnotefolder').forEach(t => {
        ...
        findAllBookChptnVersesWithNote()
        document.querySelectorAll('.verse_note .t1.text_content').forEach(t => {
            t.classList.remove('text_content') /* ; t.id=null */
        });
        document.querySelectorAll('.verse_note .t2:not(.text_content)').forEach(t => {
            t.classList.add('text_content')
        });
    ...
    if (dis) {
        ...
        if (notemenu) {
            generateNoteMenu(notemenubtn, true)
        }
    }
}
...
function removeItemFromArray(n, array) {
    if (Array.isArray(n)) {
        array.forEach((nArr, i) => {
            if (Array.isArray(nArr) && nArr[0] == n[0]) {
                array.splice(i, 1);
            }
        });
    } else {
...
function areAllitemsOfAinB_1(a, b) {
    ...
    if (Array.isArray(b)) {
        return areAllitemsOfAinB(a, b)
    } else if (a.every(elem => b.match(elem))) {
        return true
    } else {
        return false
    }
...
function isAsubArrayofB(a, b, isExactMatch = false) {
    ...
    if (bL < aL) {
        return false;
    } // if b is shorter than a, it can't contain a
...
function removeStringFromString(xh, str) {
    if (typeof xh === 'string' || xh instanceof String) {
...
async function appendMarkersToSideBar() {
    ...
    if (!document.querySelector('#currentbook_versemarkers_list')) {
        return
    }
...
function openWindow(url, windowName) {
    ...
    if (w.location.href === 'about:blank') {
        w.location.href = url;
    } // Open the window and set its URL
    w.focus();
}
/* ******************************** */
...
function enableCKEditor(ID) {
    ...
    CKEDITOR.inline(ID, {
        ...
        eTargets_note.querySelectorAll('path, rect, circle').forEach(svgElm => {
            svgElm.innerHTML = '';
        });
    });
}
...
function toggleH1to6siblings(e, eTarget, mustMatch) {
    if ((e && (!e.target.closest('H1,H2,H3,H4,H5,H6') || e.target.closest('.verse_note .notemenu') || (e.target.closest('#searchPreviewFixed,#scriptureCompareWindow') && !e.target.closest('.context_menu,.crossrefs')) || (e.type == 'contextmenu' && e.target.closest(':is([ref],.strnum,[strnum]):not(.context_menu)')))) || (mustMatch && !e.target.closest(mustMatch))) {
        return
    }
    ...
    if (e) {
        if (isMouseOverHighlightedText()) {
            ...
            return
        } // If mouse is over highlighted text, the assumption is that the intention is to copy the text
        else if (e.target.matches('[strnum]')) {
            return
        }
    ...
    if (!h1to6arr.includes(hTag.toUpperCase()) || eTargetParent.contentEditable == 'true') {
        return
    }

    function unhideAllH1to6() {
        eTargetParent.querySelectorAll('.hidingsibs').forEach(x => {
            x.classList.remove('hidingsibs');
        })
        h1to6arr.forEach(x => {
    ...
    if (e ? .type == 'contextmenu') {
        ...
        function limitedToggle(array, y = false) {
            ...
            (!y ? array : array.length > 0) ? array.forEach(x => {
                ...
                if (((!ancestors2ignore.includes(x)) || hElm_hNum <= x_hNum)) {
                    toggleH1to6siblings(null, x);
                    hid_something = true;
                }
    ...
    if (e ? .ctrlKey && e.type == 'click') {
        if (hElmHidingSiblings) {
            ...
            let allChildrenOfeTargetParent = Array.from(eTargetParent.children),
                prvHx, youMayHide = false;
            allChildrenOfeTargetParent.forEach((elm, i) => {
                ...
                if (!youMayHide && h1to6arr.includes(elmTagName)) {
                    prvHx = h1to6arr.find(x => {
                        return x == elmTagName.toUpperCase();
                    });
                    youMayHide = true;
                }
                if (youMayHide) {
                    if (!h1to6arr.includes(elmTagName) || Number(elmTagName.match(/\d+/g)) < Number(prvHx.match(/\d+/g))) {
                        h1to6arr.forEach(hx => {
                            elm.classList.remove('hidby_' + hx)
                        }); //don't hide headings
        ...
        while (hElmSibling && hElmSibTagName != hTag && hElmSibTagName != 'SCRIPT') {
            ...
            if (hElmHidingSiblings && hElmSibling.classList.contains('hidby_' + hTag)) {
                if (e && e.altKey) {
                    ...
                    hElmSibling.classList.remove('hidby_H' + (hIndx + 1))
                };
...
function distanceBetweenElementAndPreviousSibling(element) {
    ...
    if (previousSibling) {
        ...
        console.error('No previousSibling found.');
        return null;
    }
}
...
function distanceToAncestorBottom(element, ancestor) {
    if (!element || !ancestor) {
        console.error('Element or ancestor not found.');
        return null;
    }
...
function paddingDistanceOfChildToAncestor(elm, targetAncestor, parentsToIgnore) {
    if (!targetAncestor.contains(elm)) {
        return '0px'
    }
...
function closestScrollableAncestors(element, limit) {
    ...
    while (element) {
        ...
        if (isScrollableY && !scrollableAncestorY) {
            scrollableAncestorY = element;
        }
        if (isScrollableX && !scrollableAncestorX) {
            scrollableAncestorX = element;
        }
        scrollableAncestor = element;
        if ((scrollableAncestorY && scrollableAncestorX) || (element == limit)) {
            break;
        } // Found both scrollable ancestors //don't go beyond this parent
        element = element.parentElement;
    }
    return {
        elm: scrollableAncestor,
        x: scrollableAncestorX,
        y: scrollableAncestorY
    };
}
/*
// TO ZOOM ELEMENT WITH MOUSE WHEEL
let scale = 1;
 ...
 */
/* TO OBSERVE ANY CHANGE TO AN ELEMENT */
/*
...
const observer = new MutationObserver((mutationsList, observer) => {
  for (const mutation of mutationsList) {
    if (mutation.type === 'attributes') {
      ...
      console.log(`Attribute '${mutation.attributeName}' changed to '${targetElement.getAttribute(mutation.attributeName)}'`);
    }
  }
});
...
// Configure the observer to watch for attribute changes
    // childList: Set to true if you want to observe changes to the child elements within the target element.
    // attributes: Set to true if you want to observe changes to attributes of the target element.
    // characterData: Set to true if you want to observe changes to the text content of the target element.
    // subtree: Set to true if you want to include changes in the entire subtree of the target element (including nested elements).
    // attributeFilter: An array of attribute names that you want to observe. If provided, the observer will only trigger for changes to attributes listed in this array.
    // attributeOldValue: Set to true if you want the observer to record the previous value of attributes that change.
    // characterDataOldValue: Set to true if you want the observer to record the previous value of character data (text content) that changes.
const config = {attributes: true};
// Start observing the target element
observer.observe(targetElement, config);
*/
// // Function to be called when an element is added
// function handleElementAdded(mutationsList, observer) {
//     for (let mutation of mutationsList) {
//         if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
//             // New element added
//             console.log('Element added:', mutation.addedNodes[0]);
//             if(mutation.addedNodes[0].matches('#cke_noteEditingTarget')){
//                 enableInteractJSonEl(cke_noteEditingTarget, cke_noteEditingTarget)
//             };
//         }
//     }
// }
// Function to be called when an element is removed
/* function handleElementRemoved(mutationsList, observer) {
    for (let mutation of mutationsList) {
...
// const targetElementId = 'your-element-id'; // Target element ID
// const targetNode = document.getElementById(targetElementId); // Select the target node
// const targetNode = document.body; // Select the target node
// const observer = new MutationObserver(handleElementAdded); // Create an observer instance
// const config = {childList:true, subtree:false}; // Configure observer to also detect removal of nodes
// observer.observe(targetNode, config); // Start observing the target node for changes in its children

/* ******************** */
/* TensorFlow Trial *** */
/* Question from John 4 */
/* ******************** */
// let question='What is the gift of God?';
// // question='Will the true worshiper worship God in Jerusalem?'
// // question='Where will the true worshiper worship God?'
// function a2QfrmReference(question,ref,version){
//     let passage=textOfChapter(ref,version);
//     qna.load().then(model => {
//         // Find the answers
...
//         });
//     });
//     function textOfChapter(ref,version){
//         let chpt_txt='';
//         let x = bversionName;
//         bversionNameChange.value=version||bversionName;
//         getCrossReference(ref).querySelectorAll('.verse').forEach(v=>{
//             v.querySelector('code[ref]').remove();
//             chpt_txt+=v.innerText;
//         })
//         bversionNameChange.value=x;
//         console.log(chpt_txt)
//         return chpt_txt
//     }
// }
/* ******** ** ******** ********* ******* */
