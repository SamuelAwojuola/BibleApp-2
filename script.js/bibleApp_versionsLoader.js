    ...
    if (pbtn.matches('[bversion].activationDeactivationReady')) {
        ...
        setTimeout(() => {//Delayed so that the css transition on the button would have taken place on the button before the version is added or removed
            ...
            if ((pbtnCheck.checked) && (bibleversions_btns.querySelectorAll('[bversion] input:checked').length > 1)) {
                if (!document.querySelector(`.v_${vname}:not(:only-child)`)) {return}
                ...
                // If the chapter is empty, remove it (happens, e.g., when version that is removed is apocryphal version)
                let emptyChpters = document.querySelectorAll('.chptverses:has(.vmultiple:empty)');
                if (emptyChpters.length>0) {
                    emptyChpters.forEach(emptyChpt => {
                        const chptHx = emptyChpt.previousElementSibling;
                        if (chptHx?.matches('h2.chptheading')) {
                            chptHx.remove();
                        }
                        emptyChpt.remove();
                    });
                ...
                document.querySelectorAll('.chptverses:has(.vmultiples:empty)')
            ...
            // Add version from vmultiple
                ...
                if (versionsToShow.includes(vname) == 0) {
                    ...
                    allLoadedmainVerses.forEach(vrs => {
                        ...
                        let vIdx = chptNv[1];
                        let vText = window[vname]?.[bookName]?.[Number(chNumInBk) - 1]?.[Number(vIdx) - 1] ?? null;
                        if (vText) {
                            let appendHere = vrs;
...
function bibleversionDragging(e){
    if (e.target.matches('[bversion]')) {
        // e.preventDefault()
        isDragging = true;
        let dragElement = e.target;
        if (!dragElement.classList.contains('active_button')) {return}//only drag active versions
        ...
        function handleMouseMove(e) {
            ...
            if (isDragging) {
                ...
                if (otherElement && otherElement.classList.contains('active_button')) {
        ...
        function handleMouseUp() {
            ...
            if (relocatedElements.length>0){
                if (direction=='right') {
                    let elm2moveDown = dragElement.cloneNode(true);
                    dragElement.remove();
                    bibleversions_btns.insertBefore(elm2moveDown, relocatedElements[relocatedElements.length-1].nextElementSibling);
                    let shifted_version = relocatedElements[relocatedElements.length-1].getAttribute('bversion');
            ...
            function removeRelocatingClasses() {
...
/* Move Last Clicked Versions With Arrow Keys */
document.addEventListener("keydown", moveSelectedVersionWithKeys)
function moveSelectedVersionWithKeys(e){
    let activeElmOkay = (!document.activeElement.matches('input, #app_settings > button, [contenteditable="true"]') && !document.activeClickedElement.closest('[contenteditable="true"]'));

    if(!activeElmOkay)return
    
    /* SELECT VERSION TO MOVE WITH NUMBER KEY (NOT NUMBERPAD KEYS) */
    // Versions will correspond to the number pressed. E.g., 1 for first version, 2 for second version, etc. 
    if (!e.altKey && !e.shiftKey && e.code.match(/Digit/i)) {
        const digit = Number(e.code.replace(/Digit/i, '')); // Extract the digit
        if (digit >= 1 && digit <= versionsToShow.length && 
            !document.activeElement.matches('input') && 
            !document.activeClickedElement?.closest('[contenteditable="true"]')) {
                moveableBibleVersion = versionsToShow[digit - 1]; // Use digit - 1 for 0-based index
                bibleversions_btns.querySelector(`button[bversion="${moveableBibleVersion}"]`).focus();
            return;
        }
    }
    ...
    /* MOVE VERSION */
    let versionToMove;
    if (['Numpad4','ArrowLeft','Numpad6','ArrowRight'].includes(e.code) && !e.altKey && !e.shiftKey) {
        /* Version to move can be selected by making any of the button[bversion].active_button the active element. E.g., by tabbing to it.  */
        const selectedVersionBtn = document.activeElement;
        versionToMove = selectedVersionBtn.matches('#bibleversions_btns button[bversion].active_button') ? selectedVersionBtn.getAttribute('bversion') : moveableBibleVersion;
        if(!versionsToShow.includes(versionToMove)){return}
        moveableBibleVersion = versionToMove;
    } else {return}
    // Order of versionsToShow
    
    let versionsArray = [];
    bibleversions_btns.querySelectorAll('.active_button').forEach(btn => {versionsArray.push(btn.getAttribute('bversion'));});
    let vrsionIndx = versionsArray.indexOf(versionToMove);
    let direction = null;
    
    if (activeElmOkay && (e.code === 'Numpad4' || (e.code === 'ArrowLeft' && !e.altKey && !e.shiftKey && document.activeClickedElement.closest('.vmultiple .verse, #bibleversions_btns button[bversion].active_button')))) {
        if (vrsionIndx > 0) {direction='-100%';}
    }
    if (activeElmOkay && (e.code === 'Numpad6' || (e.code === 'ArrowRight' && !e.altKey && !e.shiftKey && document.activeClickedElement.closest('.vmultiple .verse, #bibleversions_btns button[bversion].active_button')))) {
        if(vrsionIndx < versionsArray.length-1){direction='100%';}
    } 
    
    if (direction) {
        let mvwkeysSTYLE, versionToReplace;
        if (direction == '-100%') {
            versionToReplace = versionsArray[vrsionIndx-1];
            mvwkeysSTYLE = `<style id="moveVersionsWithKeysStyle">
                #main .vmultiple > .v_${versionToMove}:nth-child(${vrsionIndx+1}), [bversion="${versionToMove}"].active_button:nth-child(${vrsionIndx+1}) {
                    transform: translate(-100%, 0);
                    z-index: 100 !important;
                    background: var(--vhlt3) !important;
                    box-shadow: var(--verse-hover-shadow) !important;
                    transition: transform 350ms cubic-bezier(0.5,-0.93, 1, 1) !important;
                }
                #main .vmultiple > .v_${versionToReplace}:nth-child(${vrsionIndx}), [bversion="${versionToReplace}"].active_button:nth-child(${vrsionIndx}) {
                    transform: translate(100%, 0);
            ...
            mvwkeysSTYLE = `<style id="moveVersionsWithKeysStyle">
                ...
                #main .vmultiple > .v_${versionToReplace}:nth-child(${vrsionIndx+2}), [bversion="${versionToReplace}"].active_button:nth-child(${vrsionIndx+2}) {
                    transform: translate(-100%, 0);
                    z-index: 99 !important;
                    transition: transform 150ms ease-in-out!important;
                }
        ...
        let alertStyleDocHead = new DOMParser().parseFromString(mvwkeysSTYLE, 'text/html').head;
        document.head.appendChild(alertStyleDocHead.firstElementChild);

        setTimeout(() => {
            // Perform DOM swap
            if (direction == '-100%') {
                insertElmAbeforeElmB(
                    bibleversions_btns.querySelector(`[bversion="${versionToMove}"].active_button`),
                    bibleversions_btns.querySelector(`[bversion="${versionToReplace}"].active_button`)
                );
                main.querySelectorAll(`.vmultiple`).forEach(vMltple => {
                    if(vMltple.querySelector(`.verse.v_${versionToMove}`) && vMltple.querySelector(`.verse.v_${versionToReplace}`)){
                        insertElmAbeforeElmB(
                            vMltple.querySelector(`.verse.v_${versionToMove}`),
                            vMltple.querySelector(`.verse.v_${versionToReplace}`)
                        );
                    }
                });
            } else if (direction == '100%') {
                ...
                main.querySelectorAll(`.vmultiple`).forEach(vMltple => {
                    if(vMltple.querySelector(`.verse.v_${versionToMove}`) && vMltple.querySelector(`.verse.v_${versionToReplace}`)){
                        insertElmAafterElmB(vMltple.querySelector(`.verse.v_${versionToMove}`),vMltple.querySelector(`.verse.v_${versionToReplace}`));
                    }
                });
            }

            // Update versionsToShow array
            versionsToShow[versionsToShow.indexOf(versionToReplace)] = versionToMove;
            versionsToShow[vrsionIndx] = versionToReplace;
            localStorage.setItem('versionsToShow', versionsToShow);

            // Apply reset style to clear transforms
            const resetStyle = `<style id="moveVersionsWithKeysResetStyle">
                #main .vmultiple > .v_${versionToMove}:not(only-child), [bversion="${versionToMove}"].active_button,
                #main .vmultiple > .v_${versionToReplace}:not(only-child), [bversion="${versionToReplace}"].active_button {
                    transform: none !important;
                    transition: none !important;
                }
            </style>`;
            let resetStyleDocHead = new DOMParser().parseFromString(resetStyle, 'text/html').head;
            document.head.appendChild(resetStyleDocHead.firstElementChild);

            // Remove all animation and reset styles after a brief delay to ensure reset applies
            setTimeout(() => {
                document.querySelectorAll('#moveVersionsWithKeysStyle, #moveVersionsWithKeysResetStyle').forEach(m => m.remove());
            }, 50); // Short delay to allow reset style to take effect
        }, 400); // Matches animation duration (150ms) + delay (20ms)
    }
}
document.addEventListener("keydown", (e)=>{if(e.code=='Digit2' && e.altKey){checkUncheck(enlargeSecondVersion_check)}}) // ALT + 2// Toggle enlargeSecondVersion_check